...
class GS_Model:
    ...
    def build_sr_class_block(self):
        ...
        sr_mid = tf.layers.conv2d(
            self.pl_in_data, 1, 1, 1, kernel_initializer=K_INIT(), use_bias=False)
    ...
    def build_sp_class_block(self):
        ...
        sp_mid = tf.layers.conv2d(
            self.pl_in_data, 1, 2, 1, kernel_initializer=K_INIT(), use_bias=False)
    ...
    def build_full_class_block(self):
        ...
        layerX_mid1 = tf.layers.conv2d(
            self.pl_in_data, 128, 1, 1, kernel_initializer=K_INIT(), use_bias=False)
        layerX_mid1 = tf.layers.batch_normalization(layerX_mid1, training=self.pl_training)
        layerX_mid1 = tf.nn.relu(layerX_mid1)

        self.layerX_mid1 = tf.reduce_mean(layerX_mid1, axis=[1,2])
        
        layerX_mid2 = tf.layers.conv2d(
            self.pl_in_data, 128, 1, 1, kernel_initializer=K_INIT(), use_bias=False)
        layerX_mid2 = tf.layers.batch_normalization(layerX_mid2, training=self.pl_training)
        layerX_mid2 = tf.nn.relu(layerX_mid2)
        layerX_mid2 = tf.layers.conv2d(
            layerX_mid2, 128, 2, 1, kernel_initializer=K_INIT(), use_bias=False)
        layerX_mid2 = tf.layers.batch_normalization(layerX_mid2, training=self.pl_training)
        layerX_mid2 = tf.nn.relu(layerX_mid2)

        self.layerX_mid2 = tf.reduce_mean(layerX_mid2, axis=[1,2])
        
        layerX_mid3 = tf.layers.conv2d(
            self.pl_in_data, 128, 1, 1, kernel_initializer=K_INIT(), use_bias=False)
        layerX_mid3 = tf.layers.batch_normalization(layerX_mid3, training=self.pl_training)
        layerX_mid3 = tf.nn.relu(layerX_mid3)
        layerX_mid3 = tf.layers.conv2d(
            layerX_mid3, 128, 3, 1, kernel_initializer=K_INIT(), use_bias=False)
        layerX_mid3 = tf.layers.batch_normalization(layerX_mid3, training=self.pl_training)
        layerX_mid3 = tf.nn.relu(layerX_mid3)

        self.layerX_mid3 = tf.reduce_mean(layerX_mid3, axis=[1,2])
        
        layerX_mid4 = tf.layers.conv2d(
            self.pl_in_data, 128, 1, 1, kernel_initializer=K_INIT(), use_bias=False)
        layerX_mid4 = tf.layers.batch_normalization(layerX_mid4, training=self.pl_training)
        layerX_mid4 = tf.nn.relu(layerX_mid4)
        layerX_mid4 = tf.layers.conv2d(
            layerX_mid4, 128, 4, 2, kernel_initializer=K_INIT(), use_bias=False)
        layerX_mid4 = tf.layers.batch_normalization(layerX_mid4, training=self.pl_training)
        layerX_mid4 = tf.nn.relu(layerX_mid4)

        self.layerX_mid4 = tf.reduce_mean(layerX_mid4, axis=[1,2])

        stack = tf.concat([
            self.layerX_mid1, self.layerX_mid2,
            self.layerX_mid3, self.layerX_mid4,
        ], axis=1)
        stack = tf.layers.dropout(stack, training=self.pl_training)
    ...
    def init_img_keys(self):
        ...
        self.tr_path = path.join(self.data_dir, 'training.h5')
        self.v_path = path.join(self.data_dir, 'validation.h5')

        self.tr_keys = fetch_keys(self.tr_path)
        self.v_keys = fetch_keys(self.v_path)
    ...
    def get_feeder(self, key_type, batch_size=None):
        ...
        if key_type == 'training':
            ...
            keys = self.v_keys
            data_path = self.v_path
        else:
            raise ValueError(f'wrong key_type: {key_type}, must be "validation", "training"')
    ...
    def do_validation_saving(self, sess, v_feeder, saver, ckpt_dir, ep=0):
        ...
        try:
            if L[3] < self.last_full_loss:
                print(f'current losses: {L}, last: {self.last_full_loss}, saving model')
